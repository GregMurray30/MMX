...
To build intuition, consider a long-run trend. Suppose outcomes steadily decline over time for reasons unrelated to spend, but the model does not account for it. In this case, a fixed-effects attribution model would misattribute the decline to reduced spend effectiveness, flattening the true causal curve and distorting inference.

A competent data scientist would recognize this pattern in the residuals—positive early, negative later—and add a trend component to correct for the confounder. But long-run trends are just one form of latent state: large, slow-moving, and easily spotted.

Most attribution pipelines stop there. Ours does not. We extend the same logic to model faster, irregular latent states that systematically influence outcomes—and we go one step further by hierarchically partitioning those states into paid and organic components, enabling a deeper and more accurate causal decomposition. The latent state partitioning into paid and organic is critical for properly testing the model's ability to recover causality under more diverse and realistic data generating conditions during validation.

To validate the approach, we conduct a suite of simulations across a variety of marketing environments, stress-testing the model’s ability to outperform traditional MMM and SKAN-derived response curves under different bias regimes, latent states, and spend distributions. These simulations show that, under a broad range of realistic conditions, the MMX model improves attribution quality relative to SKAN in the majority of cases, particularly when spend patterns are decorrelated and magnitudes are sufficient. However, in settings with highly correlated or very small spend, MMX can underperform, reflecting fundamental identifiability limits rather than modeling error.
