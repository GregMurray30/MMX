...
data {
  ...
  row_vector[N_train] event_1_train;
  row_vector[N_train] event_2_train;
  row_vector[N_train] event_3_train;
  row_vector[N_train] event_4_train;
  ...
  real beta_event_1_mn;
  real beta_event_1_sd;
  real beta_event_2_mn;
  real beta_event_2_sd;
  real beta_event_3_mn;
  real beta_event_3_sd;
  real beta_event_4_mn;
  real beta_event_4_sd;
...
parameters {
  ...
  real<lower=0> beta_event_1;
  real<lower=0> beta_event_2;
  real<lower=0> beta_event_3;
  real<lower=0> beta_event_4;
...
transformed parameters {
  ...
  for (n in 1:N_train) {
    real season_trend = 1 + common_trend_train[n] +
      beta_event_1 * event_1_train[n] +
      beta_event_2 * event_2_train[n] +
      beta_event_3 * event_3_train[n] +
      beta_event_4 * event_4_train[n] ;
...
model {
  nu ~ normal(nu_prior_mn, nu_prior_sd);

  sigma_add ~ normal(sigma_mult_prior_mn, sigma_mult_prior_sd);
 // sigma_mult ~ normal(sigma_add_prior_mn, sigma_add_prior_sd);


  beta_event_1 ~ normal(beta_event_1_mn, beta_event_1_sd);
  beta_event_2 ~ normal(beta_event_2_mn, beta_event_2_sd);
  beta_event_3 ~ normal(beta_event_3_mn, beta_event_3_sd);
  beta_event_4 ~ normal(beta_event_4_mn, beta_event_4_sd);
   ...
   // Y_true_train[n] ~ normal(mu_train[n] + shared_shock[n], sigma_add + softplus((mu_train[n]+.01)*sigma_mult));
    ...
    for (c in 1:num_media) {
      ...
      for (j in 1:num_media) {
        if (j != c) {
          cannib_term += cannibalization_effect[j] * log1p(fmax(spend_train[n, j], 1e-2));
        }
      }
